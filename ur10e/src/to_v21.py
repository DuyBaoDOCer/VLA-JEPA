...
def _extract_video_segment(
    ...
    #
    # Re-encode instead of stream-copy (-c copy). -c copy can only cut on
    # keyframe boundaries, so a cut can silently land up to one GOP away from
    # the requested timestamp -- exactly the phase-misalignment risk this
    # pack exists to rule out. Re-encoding every frame guarantees the output
    # starts and ends at the exact requested timestamps.
    cmd = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel",
